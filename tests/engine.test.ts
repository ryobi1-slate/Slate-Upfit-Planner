...
describe('geometry helpers', () => {
	...
	it('finds a legal open placement on an empty driver wall', () => {
		const spot = findOpenPlacement(shelf48, driver, [], COMPONENTS_BY_SKU);
...
describe('validatePlacement', () => {
	...
	it('rejects overlap with another shelf', () => {
		...
		expect(issues.some((i) => i.code === 'SHELF_COLLISION')).toBe(true);
	});
	...
	it('rejects the partition (front boundary) zone', () => {
		...
		expect(issues.some((i) => i.code === 'STARTS_IN_PARTITION')).toBe(true);
	});
	...
	it('rejects extending past the rear boundary', () => {
		...
		expect(issues.some((i) => i.code === 'EXCEEDS_CARGO')).toBe(true);
	});
	...
	it('rejects the passenger no-mount contoured inset', () => {
		...
		expect(issues.some((i) => i.code === 'BLOCKED_ZONE')).toBe(true);
	});
	...
	it('rejects blocking the sliding door', () => {
		...
		expect(issues.some((i) => i.code === 'DOOR_CONFLICT')).toBe(true);
	});
	...
	it('rejects a roof-incompatible vehicle', () => {
		...
		expect(issues.some((i) => i.code === 'INCOMPATIBLE_VEHICLE')).toBe(true);
	});
});
...
describe('interaction flow (engine + reducer, mirrors the UI hook)', () => {
	it('places, snaps/clamps a move, flags overlap, preserves on wall switch, removes', () => {
		...
		state = plannerReducer(state, { type: 'SELECT_PRODUCT', sku: '22-3438' });
		const spot = findOpenPlacement(shelf48, driver, [], COMPONENTS_BY_SKU)!;
		...
		expect(conflicts.some((i) => i.code === 'SHELF_COLLISION')).toBe(true);

		// 4. Switch wall preserves both placements.
		state = plannerReducer(state, { type: 'SWITCH_WALL', wall: 'passenger' });
