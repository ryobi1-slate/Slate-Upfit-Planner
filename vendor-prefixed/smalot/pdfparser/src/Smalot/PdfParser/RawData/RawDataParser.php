 ...
 */
namespace Slate\UpfitPlanner\Vendor\Smalot\PdfParser\RawData;
...
use Slate\UpfitPlanner\Vendor\Smalot\PdfParser\Exception\MissingPdfHeaderException;
class RawDataParser
{
    /**
     * @var Config
    ...
    protected function decodeStream(string $pdfData, array $xref, array $sdic, string $stream) : array
    {
        ...
        foreach ($filters as $filter) {
            if (\in_array($filter, $this->filterHelper->getAvailableFilters(), \true)) {
                try {
    ...
    protected function getRawObject(string $pdfData, int $offset = 0, ?array $headerDic = null) : array
    {
        ...
        switch ($char) {
            ...
            case '/':
                ...
                $objtype = $char;
                ++$offset;
                $span = \strcspn($pdfData, "\x00\t\n\f\r \n\t\r\v\f()<>[]{}/%", $offset, 256);
                if ($span > 0) {
                    $objval = \substr($pdfData, $offset, $span);
                    // unescaped value
                    $offset += $span;
                }
                break;
            ...
            case '>':
                ...
                if (isset($pdfData[$offset + 1]) && $pdfData[$offset + 1] == $char) {
                    ...
                    $objtype = $char;
                    ++$offset;
                    $span = \strspn($pdfData, "0123456789abcdefABCDEF\t\n\f\r ", $offset);
            ...
            default:
                if ('endobj' == \substr($pdfData, $offset, 6)) {
                    ...
                    if (1 == \preg_match('/^( *[\\r]?[\\n])/isU', \substr($pdfData, $offset, 4), $matches)) {
                        ...
                        $pregResult = \preg_match('/(endstream)[\\x09\\x0a\\x0c\\x0d\\x20]/isU', $pdfData, $matches, \PREG_OFFSET_CAPTURE, $offset + $streamLen);
                        if (1 == $pregResult) {
    ...
    private function getHeaderValue(?array $headerDic, string $key, string $type, $default = '')
    {
        ...
        foreach ($headerDic as $i => $val) {
            ...
            if ($isSectionName && $val[1] == $key && isset($headerDic[$i + 1])) {
                $isSectionValue = \is_array($headerDic[$i + 1]) && 1 < \count($headerDic[$i + 1]);
                return $isSectionValue && $type == $headerDic[$i + 1][0] ? $headerDic[$i + 1][1] : $default;
            }
        }
        return $default;
    }
    ...
    protected function getXrefData(string $pdfData, int $offset = 0, array $xref = [], array $visitedOffsets = []) : array
    {
        ...
        if (\in_array($offset, $visitedOffsets, \true)) {
        ...
        $startxrefPreg = \preg_match_all('/(?<=[\\r\\n])startxref[\\s]*[\\r\\n]+([0-9]+)[\\s]*[\\r\\n]+%%EOF/i', $pdfData, $startxrefMatches, \PREG_SET_ORDER, $offset);
        if (0 == $startxrefPreg) {
