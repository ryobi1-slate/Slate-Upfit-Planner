...
use Slate\UpfitPlanner\Vendor\Smalot\PdfParser\Encoding\WinAnsiEncoding;
    ...
    public function decodeText(array $commands, float $fontFactor = 4) : string
    {
        ...
        foreach ($commands as $command) {
            switch ($command[PDFObject::TYPE]) {
                case 'n':
                    $offset = (float) \trim($command[PDFObject::COMMAND]);
        ...
        $words = \str_replace([" \x00\x00 ", "\x00\x00 ", " \x00\x00", "\x00\x00"], ['  ', ' ', ' ', ' '], $words);
        return $words;
    }
    /**
