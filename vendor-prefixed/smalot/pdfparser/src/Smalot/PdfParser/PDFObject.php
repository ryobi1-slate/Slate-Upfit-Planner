 ...
 */
namespace Slate\UpfitPlanner\Vendor\Smalot\PdfParser;

use Slate\UpfitPlanner\Vendor\Smalot\PdfParser\Exception\InvalidDictionaryObjectException;
use Slate\UpfitPlanner\Vendor\Smalot\PdfParser\XObject\Form;
use Slate\UpfitPlanner\Vendor\Smalot\PdfParser\XObject\Image;
/**
 * Class PDFObject
 */
class PDFObject
{
    ...
    private function formatContent(?string $content) : string
    {
        ...
        while (\preg_match('/\\sBI\\s(\\/.+?)\\sID\\s(.+?)\\sEI(?=\\s|$)/s', $content, $text, \PREG_OFFSET_CAPTURE, $offsetBI)) {
            ...
            if ((isset($dict['W']) || isset($dict['Width'])) && (isset($dict['H']) || isset($dict['Height']))) {
                $id = \uniqid('IMAGE_', \true);
                $pdfInlineImages[$id] = [\preg_replace(['/\\r\\n/', '/\\r/', '/\\n/'], ' ', $text[1][0]), \preg_replace(['/\\r\\n/', '/\\r/', '/\\n/'], '', $text[2][0])];
        ...
        $pdfInlineImages = \array_reverse($pdfInlineImages, \true);
    ...
    public function getSectionsText(?string $content) : array
    {
        ...
        $textCleaned = \preg_split('/(\\r\\n|\\n|\\r)/', $this->formatContent($content), -1, \PREG_SPLIT_NO_EMPTY);
        $inTextBlock = \false;
    ...
    public function parseDictionary(string $dictionary) : array
    {
        ...
        $split = \preg_split('/(<<|>>|\\[|\\]|\\/[^\\s\\/\\[\\]\\(\\)<>]*)/', \trim(\preg_replace('/^<<|>>$/', '', $dictionary)), -1, \PREG_SPLIT_NO_EMPTY | \PREG_SPLIT_DELIM_CAPTURE);
    ...
    {
        $this->addPositionWhitespace = \true;
                ...
                switch ($command[self::OPERATOR]) {
                        ...
                        if (\preg_match('/(<<.*>>)$/', $command[self::COMMAND], $match)) {
                            ...
                            if (isset($dict['ActualText']) && \is_string($dict['ActualText']) && '' !== $dict['ActualText']) {
                                if ('[' == $dict['ActualText'][0]) {
                                    ...
                                    $marked_stack[] = ['ActualText' => $this->getCommandsText($dict['ActualText'] . 'TJ')[0]];
                    ...
                    case 'BMC':
                        if ('ReversedChars' == $command[self::COMMAND]) {
                            ...
                            $marked_stack[] = ['ReversedChars' => \true];
                        }
                        break;
                    ...
                    case 'Do':
                        ...
                        if (\in_array($xobject->getUniqueId(), self::$recursionStack, \true)) {
                            break;
                        }
                    ...
                    case 'TJ':
                        ...
                        if (\true === $this->addPositionWhitespace && \false !== $current_position['x']) {
                        ...
                        $newtext = $this->getTJUsingFontFallback($current_font, $command[self::COMMAND], $page, $factorX);
                        ...
                        if (\false === $actual_text) {
                            ...
                            $factor = $factorX / 2;
                            $current_position = ['x' => $currentX - \mb_strlen($newtext) * $factor, 'y' => $currentY];
        ...
        if ('TJ' == $operator) {
            ...
            do {
                ...
                if (\preg_match('/^ *\\((.*?)(?<![^\\\\]\\\\)\\) *(-?[\\d.]+)?/', $command, $tjmatch)) {
                    ...
                    if (isset($tjmatch[2]) && \trim($tjmatch[2])) {
    ...
    public static function factory(Document $document, Header $header, ?string $content, ?Config $config = null) : self
    {
        switch ($header->get('Type')->getContent()) {
            ...
            case 'Font':
                ...
                $classname = '\\Slate\\UpfitPlanner\\Vendor\\Smalot\\PdfParser\\Font\\Font' . $subtype;
